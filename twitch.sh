...
#	echo $streamer
	...
	if [[ ! -d $destpath/twitch/$streamer ]]; then
		mkdir $destpath/twitch/$streamer
	fi
...
fnRequest(){
	...
	#		echo "Cleared config... returning to config."
			fnTwitchConfig
		elif [[ -z $(ps -ef | grep -v grep | grep "https://www.twitch.tv/$streamer" | grep streamlink) ]]; then
			#If we aren't already recording, and the game they're playing matches what we want to record, then start recording.
			fnStartRecord
		else
			echo "[-] $streamer is not live. $request"
		fi



	elif [[ $(echo $request | jq -r '.error') != "null" || -z $access_token ]]; then
#		echo "Token Error: $(echo $request | jq -r '.error') ---- $access_token"
		unset access_token
		rm $configfile
#		echo "Cleared config... returning to config."
